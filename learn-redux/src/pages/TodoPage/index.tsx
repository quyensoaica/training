...
const TodoPage = () => {
  ...
  const { todoItem, action } = useSelector((state: any) => state.todo); // Call the useSelector function
  const handleSave = () => {
    if (!todoItem.title) {
    ...
    dispatch(todoAction.setAction("create"));
    dispatch(todoAction.setTodoItem({ id: "", title: "", completed: false }));
  };
  useEffect(() => {
    ...
    <div className='h-[600px] max-w-[1000px] w-[100%] shadow-xl bg-gradient-to-br from-violet-200 to-pink-200 rounded-2xl p-3 text-left'>
      <div className=''>
        <input
          ...
          onChange={(e) => dispatch(todoAction.setTodoItem({ ...todoItem, title: e.target.value }))}
